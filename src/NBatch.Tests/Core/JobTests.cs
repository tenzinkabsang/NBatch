...
internal sealed class JobTests
{
    ...
    [Test]
    public void AfterJob_listener_called_on_cancellation_with_Cancelled_true()
    {
        var listener = new RecordingJobListener();
        using var cts = new CancellationTokenSource();
        cts.Cancel();

        var job = Job.CreateBuilder("cancel-listener")
        ...
        Assert.That(listener.AfterResult!.Cancelled, Is.True);
        Assert.That(listener.AfterResult!.Success, Is.False);
    }

    [Test]
    public async Task JobResult_Cancelled_is_false_on_success_and_failure()
    {
        var okJob = Job.CreateBuilder("not-cancelled-ok")
            .AddStep("s1", step => step.Execute(() => { }))
            .Build();
        var okResult = await okJob.RunAsync();
        Assert.That(okResult.Cancelled, Is.False);

        var failJob = Job.CreateBuilder("not-cancelled-fail")
            .AddStep("s1", step => step
                .ReadFrom(new FailingReader<string>())
                .WriteTo(new CollectingWriter<string>()))
            .Build();
        var failResult = await failJob.RunAsync();
        Assert.That(failResult.Cancelled, Is.False);
        Assert.That(failResult.Success, Is.False);
    }

    #endregion

    #region Configuration ordering

    [Test]
    public async Task Steps_execute_in_registration_order_not_name_order()
    {
        var executionOrder = new List<string>();

        var job = Job.CreateBuilder("ordered-job")
            .AddStep("z-first", step => step.Execute(() => { executionOrder.Add("z-first"); }))
            .AddStep("a-second", step => step.Execute(() => { executionOrder.Add("a-second"); }))
            .AddStep("m-third", step => step.Execute(() => { executionOrder.Add("m-third"); }))
            .Build();

        var result = await job.RunAsync();

        Assert.That(result.Success, Is.True);
        Assert.That(executionOrder, Is.EqualTo(new[] { "z-first", "a-second", "m-third" }));
    }

    [Test]
    public async Task WithLogger_after_AddStep_is_honored()
    {
        var logger = new RecordingLogger();

        var job = Job.CreateBuilder("late-logger")
            .AddStep("s1", step => step
                .ReadFrom(new ListReader<string>(["a"]))
                .WriteTo(new CollectingWriter<string>()))
            .WithLogger(logger)
        ...
        Assert.That(logger.Messages, Has.Some.Contains("Step 's1' starting"),
            "steps registered before WithLogger must still use the configured logger");
    }
